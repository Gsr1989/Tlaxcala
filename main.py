...
seg  = max(0, int(TOTAL_MINUTOS_TIMER60-(datetime.now()-timers_activos[f]["start_time"]).total_seconds()))
...
{ROBOTO}{FA}<style>{CSS}</style></head><body>"""  def _sidebar_links():
return """
...
modal_html = f"""<div class="modal-overlay" id="mD">
  ...
  <div class="modal-box">  
    ...
    return HTMLResponse(page("Folios","Folios Registrados — Tlaxcala", contenido))  @app.post("/panel/validar/{folio}")
...
disp = asig - usad; porc = round((usad/asig100) if asig else 0, 1)
tz = ZoneInfo(TZ); hoy = datetime.now(tz).strftime("%Y-%m-%d")
msg = request.query_params.get("msg",""); err = request.query_params.get("error","")
...
<footer style="margin-top:30px;background:#1a1a1a;color:#aaa;padding:18px;text-align:center;font-size:12px">  
  © Gobierno del Estado de Tlaxcala 2026 — Secretaría de Movilidad y Transporte  
        ...
        return HTMLResponse(f"<p>Error: {e}</p>", status_code=500)  ===================== TABLAS BD =====================
